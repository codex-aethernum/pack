...
ветка `codex/2.4.2-aquamirae`: основана на `50990e9d6` — коммите, из которого
собран релизный джарник. Поверх — один коммит на четыре файла. Сборка из этой
ветки проверена: те же 1200 файлов, что в релизе, и единственные содержательные
отличия — наши четыре конфига (остальное — CRLF против LF и порядок constant
pool в трёх анонимных классах, байткод которых идентичен).

> База искалась сверкой ресурсов с самим джарником, а не по `mod_version`:
> версию бампают **до** релиза, и наивный «последний коммит с 2.4.2» уводит на
> четыре послерелизных коммита вперёд. Подробности — в `CODEX-FORK.md`.
...
## Сшивка IE не работала до 19 сентября вечера

Раздел выше про «KubeJS: скрипты не загружались» чинил `{ count }` на строке
1032 — но на 1045 осталось второе сокращение, `({ id, count: n })`, и
`10_ie_stitch.js` **так и не загружался ни разу**. Все 1611 строк рецептов IE
до этого дня были мёртвы. Заметно только по отсутствию строки
`Loaded script server_scripts:10_ie_stitch.js` в `logs/kubejs/server.log`.

Честный поиск сокращений (шаблонные `${…}` не считаются):

```
(?<!\$)\{\s*[A-Za-z_]\w*\s*[,}]
```

Когда сшивка ожила, она показала 69 расхождений с форматами IE 12.4.2 и
Botania 457 — все взяты из JSON самих джарников, не по памяти:

| Что | Формат IE 12 | Откуда |
|---|---|---|
| `arc_furnace` | ключ `additives` обязателен, хотя бы `[]` | `recipe/arcfurnace/alloy_brass.json` |
| рендер `stem` | `crop`, `stem`, `attachedStem` — выводятся из семян | `recipe/cloche/pumpkin.json` |
| рендер `doublecrop` | `doublingAge` (у конопли 3) | `recipe/cloche/hemp.json` |
| лепестки Botania | `botania:<цвет>_mystical_petal`, не `<цвет>_petal` | `assets/botania/models/item/` |

Итог после правок: `Added 310 recipes, removed 17, modified 2, 0 failed`.

**Как не обмануться логом.** `latest.log` пишется заново только при
перезапуске *игры*. Перезаход в мир дописывает в тот же файл — и вчерашние
ошибки выглядят сегодняшними. Сверять по времени в строке.

**Что осталось в логе и не наше:** 46 ошибок RecipeManager от Create Framed,
Malum, Eternal Tales — их собственные сломанные рецепты, см. issue в `pack`.
Плюс 578 ошибок лут-таблиц (Railways × TFC) и 38 Moonlight — были до нас,
цифры совпадают с логом сессии 15:16.
